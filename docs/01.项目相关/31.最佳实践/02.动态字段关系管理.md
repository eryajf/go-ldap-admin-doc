...
> `注意：`通常在ldap中的用户名以及分组名都尽量不应该是中文，后台默认给每个部门名称和用户名称添加了一个对应中文内容的拼音，以供用户选择。
...
- User
  - `username：`用户名，建议用 `name_pinyin` 字段映射。
...
```json
{
    ...
    "attributes": {  // 字段属性
        "groupName":"name_pinyin",  // 分组名称（通常为分组名的拼音）
        "remark":"name",  // 分组描述
...
```json
{
    ...
    "attributes": { // 字段属性
        "username":"name_pinyin", // 用户名(通常为用户名拼音)
        "nickname":"name",  // 中文名字
        "givenName":"name", // 花名
        "mail":"email",  // 邮箱
